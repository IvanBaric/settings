...
- Corexis UUID handling through `HasUuid`
...
- `SettingsModels` for validating configured model and table names
...
The `Setting` model uses `IvanBaric\Corexis\Concerns\HasUuid`, so UUID generation and route-key behavior are owned by Corexis rather than duplicated in this package.

The table name can be overridden before the first migration run:
